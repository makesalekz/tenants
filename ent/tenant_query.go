...
func (tq *TenantQuery) QueryMembers() *MemberQuery {
...
func (tq *TenantQuery) WithMembers(opts ...func(*MemberQuery)) *TenantQuery {
	query := (&MemberClient{config: tq.config}).Query()
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withMembers; query != nil {
		if err := tq.loadMembers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Members = []*Member{} },
			func(n *Tenant, e *Member) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TenantQuery) loadMembers(ctx context.Context, query *MemberQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Member)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(member.FieldTenantID)
	}
...
func (tq *TenantQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Group)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
