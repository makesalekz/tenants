...
CREATE UNIQUE INDEX "invite_tenant_id_email_status" ON "invites" ("tenant_id", "email", "status") WHERE ((status)::text = 'accepted'::text);
-- Create index "invite_tenant_id_user_id_status" to table: "invites"
CREATE UNIQUE INDEX "invite_tenant_id_user_id_status" ON "invites" ("tenant_id", "user_id", "status") WHERE ((status)::text = 'accepted'::text);
